...
def create_art(nails, orig_pic, str_pic, str_strength, random_nails=None, i_limit=None):
...
def create_string_art(input_file, side_len=300, export_strength=0.1,
                      pull_amount=None, random_nails=None, radius1_multiplier=1, radius2_multiplier=1,
                      nail_step=4, wb=False, rgb=False, rect=False):
    LONG_SIDE = 300
    ...
    if rgb:
        iteration_strength = 0.05 if wb else -0.05
        ...
        pull_orders_r = create_art(nails, r, str_pic_r, iteration_strength, i_limit=pull_amount)

        str_pic_g = init_canvas(shape, black=wb)
        pull_orders_g = create_art(nails, g, str_pic_g, iteration_strength, i_limit=pull_amount)

        str_pic_b = init_canvas(shape, black=wb)
        pull_orders_b = create_art(nails, b, str_pic_b, iteration_strength, i_limit=pull_amount)
        ...
        if wb:
            ...
            pull_order = create_art(nails, orig_pic, str_pic, 0.05, i_limit=pull_amount)
